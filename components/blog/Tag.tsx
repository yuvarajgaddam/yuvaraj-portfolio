...
const Tag: React.FC<Props> = ({ tag, size = "sm", count }) => {
  return (
    <Link
      ...
      key={tag}
      passHref
      className={`blog-tag link-outline group whitespace-nowrap dark:fill-bglight hover:bg-marrsyellow hover:text-bglight hover:fill-bglight dark:hover:bg-carriyellow dark:hover:text-bgdark dark:hover:fill-bgdark ${
        size === "lg"
